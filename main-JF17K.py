...
class Experiment:
    def __init__(self, num_iterations, batch_size, lr, dr, dembd, max_ary, layer_num, ary_list):
            ...
            if len(test_data_idxs[ary - 2]) > 0:
                for miss_ent_domain in range(1, ary + 1):
                    ...
                    for i in range(0, len(test_data_idxs[ary - 2]), self.batch_size):
                        ...
                        pred, _ = model.forward(r_idx, e_idx, miss_ent_domain)
    ...
    def train_and_eval(self):
        ...
        model = HMTE(len(d.ent2id), len(d.rel2id), self.dembd, self.max_ary, self.device, self.layer_num,
                     ent_relnel=d.ent_relnel, ary_list=self.ary_list)
        model.to(device)
        ...
        best_valid_iter = 0
        best_valid_metric = {'valid_mrr': -1, 'test_mrr': -1, 'test_hit1': -1, 'test_hit3': -1, 'test_hit10': -1,
                             'group_test_mrr': [], 'group_test_hits': []}
        ...
        for it in range(1, self.num_iterations + 1):
                ...
                for miss_ent_domain in range(1, ary + 1):
                    ...
                    for j in range(0, len(er_vocab_pairs), self.batch_size):
                        data_batch, label, rel_idx, ent_idx = self.get_batch(er_vocab, er_vocab_pairs, j,
                                                                             miss_ent_domain)
                        pred, loss_b = model.forward(rel_idx, ent_idx, miss_ent_domain)
                        pred = pred.to(device)
                        loss = model.loss(pred, label) + loss_b
            ...
            if it % param['eval_step'] == 0:
                ...
                with torch.no_grad():
                    ...
                    print('Epoch=%d, Valid MRR=%f' % (it, v_mrr))
                    print('~~~~~~~~~~~~~ Test ~~~~~~~~~~~~~~~~')

                    t_mrr, t_hit10, t_hit3, t_hit1, group_mrr, group_hits = self.evaluate(model, d.test_facts,
                                                                                          args.ary_list)
                    ...
                    if v_mrr >= best_valid_metric['valid_mrr']:
                        ...
                        best_valid_metric['test_hit10'], best_valid_metric['test_hit3'], best_valid_metric[
                            'test_hit1'] = t_hit10, t_hit3, t_hit1
                        ...
                        print('Valid MRR didnt increase for %d epochs, Best_MRR=%f' % (
                        it - best_valid_iter, best_valid_metric['test_mrr']))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default="JF17K", nargs="?",
                        help="FB-AUTO/JF17K/WikiPeople/WN18RR/FB15K-237.")
    ...
    parser.add_argument("--dembed", type=int, default=1024, nargs="?")
    parser.add_argument("--layer_num", type=int, default=6, nargs="?", help="layer_num.")
    ...
    param['num_iterations'], param['eval_step'], param[
        'valid_patience'] = args.num_iterations, args.eval_step, args.valid_patience
    param['batch_size'] = args.batch_size
    param['lr'], param['dr'] = args.lr, args.dr
    param['dembed'] = args.dembed
    param['layer_num'] = args.layer_num
    param['ary_list'] = args.ary_list
    ...
    Exp = Experiment(num_iterations=param['num_iterations'], batch_size=param['batch_size'], lr=param['lr'],
                     dr=param['dr'],
                     dembd=param['dembed'], max_ary=d.max_ary, layer_num=param['layer_num'],
                     ary_list=args.ary_list)
