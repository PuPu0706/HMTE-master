...
class Data:
    def __init__(self, data_dir='./data/JF17K/'):
        ...
        train_strfacts, valid_strfacts, test_strfacts, rel_list, ent_list, self.max_ary = self.load_strfacts(
            data_dir=data_dir)
        ...
        self.ent_relnel = self.getneigh(train_facts)
    ...
    def getneigh(self, train_facts):
        MAX_NEIGHBORS = 100
        neighbor_dict = defaultdict(Counter)
        for triples in train_facts:
            for triple in triples:
                entities = triple[1:]
                for entity in entities:
                    for neighbor in entities:
                        if entity != neighbor:
                            neighbor_dict[entity][neighbor] += 1

        neighbor_list = {}
        for entity, counter in neighbor_dict.items():
            sorted_neighbors = [neighbor for neighbor, _ in counter.most_common(MAX_NEIGHBORS)]
            neighbor_list[entity] = sorted_neighbors
    ...
    def load_strfacts(self, data_dir='./data/JF17K/'):
        ...
        return train_strfacts, valid_strfacts, test_strfacts, sorted(list(set(rel_list))), sorted(
            list(set(ent_list))), max_ary
